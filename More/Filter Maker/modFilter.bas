...
Private Function Token(expr, pos)
    ...
    Do Until pos > Len(expr)
        ...
        Select Case fn
         ...
         Case "red"
          arr = Split(value, ",")
          Token = GetRGB(frmFilterMaker.picUndo.Point(Eval(Trim(arr(0))), Eval(Trim(arr(1))))).Red
         Case "green"
          arr = Split(value, ",")
          Token = GetRGB(frmFilterMaker.picUndo.Point(Eval(Trim(arr(0))), Eval(Trim(arr(1))))).Green
         Case "blue"
          arr = Split(value, ",")
          Token = GetRGB(frmFilterMaker.picUndo.Point(Eval(Trim(arr(0))), Eval(Trim(arr(1))))).Blue
